...
### triple hash key represents IMPORTANT README ###
...
#the lat lon are coordinates to obtain partial data in the shape of a square, keeping only flight paths of interest
#"all" for all flights, "gap start" for flights containing arrivals, "gap end" for containing departures
#plot for selceted aircraft, values = c(0 for arrivals, 1 for departures). if you want to plot ALL routes, use SphereContinuousAirportBillboardSelectSquareAnnotate.R
targetair<-"WSSS"
getTarget("all",targetair)
plot <- 1
filename<-"SphereContinuousAirportBillboardSelectAnnotateDepartures.txt"
latmax<-3
latmin<-0
longmax<-105
longmin<-102.5
...
####target is in the form of a list of aircraft e.g.target <- 0, target[1] <- "75047c", target[2] <- "75027d"###
...
###only use this if want select an area of flights###
air <- air[which(air$latitude<latmax & air$latitude>latmin & air$longitude<longmax & air$longitude>longmin),]###
...
for (i in 1:length(target)){
  ...
  if (max(air$splitted)>1){
    for (j in 1:(max(air$splitted)-1)){
      ...
      if (plot==0){
        if (miniair$nearestairport[nrow(miniair)]=="Singapore Changi Airport" | miniair$nearestairport[nrow(miniair)]=="Changi Air Base (East)"){
          miniair <- miniair[which(miniair$latitude<latmax & miniair$latitude>latmin & miniair$longitude<longmax & miniair$longitude>longmin),] ###
          
          #only do if there are entries remaining after we cut the square
          if(nrow(miniair)!=0){
    ...
    position : {
        ...
        if (miniair$nearestairport[1]=="Singapore Changi Airport" | miniair$nearestairport[1]=="Changi Air Base (East)"){
          miniair <- miniair[which(miniair$latitude<latmax & miniair$latitude>latmin & miniair$longitude<longmax & miniair$longitude>longmin),] ###
...
#add the annotate functionality ref: https://sandcastle.cesium.com/?src=3D%20Tiles%20Interactivity.html
...
function makeProperty(entity) {
	entity.path.width = 50;
	entity.path.material.color._value = Cesium.Color.RED;
}
...
handler.setInputAction(function (movement) {
  ...
  var pickedd = picked.id;
  makeProperty(pickedd)
